...
def main():
    ...
    while True:
        try:
            ...
            # -------------------------
            if not fixture:
                print("No live Norwich match — sleeping 30 minutes", flush=True)
                time.sleep(1800)
                continue
            ...
            if not match_state["lineups_posted"] and status in ("NS", "1H"):
                ...
                for team in lineups:
                    if team.get("team", {}).get("name") == home:
                        ...
                        ref = safe_post(
                            f"📋 Norwich City XI ({team.get('formation')})\n\n"
                            + "\n".join(starters)
                            + "\n\n#NCFC",
                            state,
                        )
            ...
            # -------------------------
            # Event fetch logic (tight)
            # -------------------------
            fetch_events = False

            if score != match_state["last_score"]:
                fetch_events = True
            elif minute in (45, 90):
                fetch_events = True
            elif minute >= 80 and POLL_COUNT % 2 == 0:
                fetch_events = True
